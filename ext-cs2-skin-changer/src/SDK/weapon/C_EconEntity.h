...
void SetMeshMask(const uintptr_t ent, const uint64_t mask)
{
    ...
    if (!node || node < 0x10000) return;

    const uintptr_t model = node + Offsets::m_modelState;

    const uintptr_t dirtyAttributes = mem.Read<uintptr_t>(model + Offsets::m_pDirtyModelData);
    if (dirtyAttributes && dirtyAttributes > 0x10000)
        mem.Write<uint64_t>(dirtyAttributes + Offsets::m_DrityMeshGroupMask, mask);

    // Máximo 3 tentativas — evita loop infinito quando entidade é destruída no lobby
...
void UpdateWeapons(const std::vector<uintptr_t> weapons)
{
    ...
    // Clean up fallback paint kits — but skip knives so their skin persists
    for (const uintptr_t& weapon : weapons)
    {
        ...
        // Don't wipe the knife's paint kit — it was set intentionally
