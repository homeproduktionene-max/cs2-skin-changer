...
namespace SC_GUI {
        ...
        Image* Get(const std::string& key, const std::string& url, bool diskEnabled) {
            ...
            if (!IsDownloading(key) && !url.empty()) {
                ...
                std::thread([this, key, url, diskEnabled]() {
                    ...
                    // Parse URL: expect "https://host/path"
                    std::string dlUrl = url;
                    std::wstring wHost, wPath;
                    {
                        // Strip scheme
                        std::string tmp = dlUrl;
                        if (tmp.substr(0, 8) == "https://") tmp = tmp.substr(8);
                        else if (tmp.substr(0, 7) == "http://") tmp = tmp.substr(7);
                        size_t slash = tmp.find('/');
                        std::string host = (slash != std::string::npos) ? tmp.substr(0, slash) : tmp;
                        std::string path = (slash != std::string::npos) ? tmp.substr(slash) : "/";
                    ...
                    HINTERNET hSess = WinHttpOpen(L"SkinChanger/1.0",
                            ...
                            if (hReq) {
                                ...
                                if (WinHttpSendRequest(hReq, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
                                    ...
                                    do {
                                        ...
                                        size_t offset = imgData.size();
                                        imgData.resize(offset + dwSize);
                                        DWORD dwRead = 0;
                                        WinHttpReadData(hReq, imgData.data() + offset, dwSize, &dwRead);
                                        imgData.resize(offset + dwRead);
                                    } while (dwSize > 0);
                                    dlOk = !imgData.empty();
                                }
                    ...
                    if (dlOk) {
                        ...
                        if (CreateStreamOnHGlobal(NULL, TRUE, &pStream) == S_OK) {
                            pStream->Write(imgData.data(), (ULONG)imgData.size(), NULL);
    ...
    inline bool TabButton(const std::string& id, const std::string& text, float x, float y, float w, float h, const std::string& icon, bool active) {
        return TabButton(id, text, x, y, w, h, active, icon);
    }
    ...
    inline bool SkinCard(const std::string& id, const std::string& name, const std::string& image_url, float x, float y, float w, float h, bool selected) {
        return SkinCard(id, name, image_url, 1, x, y, w, h, selected, true);
    }

    inline bool SkinCard(const std::string& id, const std::string& name, const std::string& image_url, float x, float y, float w, float h, bool selected, bool diskEnabled) {
        return SkinCard(id, name, image_url, 1, x, y, w, h, selected, diskEnabled);
    }
